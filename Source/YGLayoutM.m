...
- (BOOL)isLeaf
{
  ...
  if (self.isEnabled) {
    for (ZDFlexLayoutView subview in self.view.children) {
      YGLayoutM *const yoga = subview.yoga;
  ...
  YGMeasureMode heightMode)
{
  ...
  if (!view.yoga.isUIView || [view.children count] > 0) {
...
{
  YGLayoutM *const yoga = view.yoga;
...
static void YGApplyLayoutToViewHierarchy(ZDFlexLayoutView view, BOOL preserveOrigin)
{
  ...
  const YGLayoutM *yoga = view.yoga;
